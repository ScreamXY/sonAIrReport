...
const calculateCost = (inputTokens: number, outputTokens: number, inputPrice: number, outputPrice: number): number => {
...
Extract data for EVERY project visible across ${imageCount > 1 ? 'all screenshots' : 'the screenshot'}. Be precise and accurate.`;
  ...
  const content: MessageContent[] = [{ type: 'text', text: userPrompt }];
